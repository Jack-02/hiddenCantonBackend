...
from spot.models import Spot, Route, Video, Audio, SpotInRoute, Image
...
def info(req: HttpRequest):
    ...
    if req.method == "GET":
        ...
        name, location, text, quote, image_titles, image_cloudids, video_titles, video_cloudids, audio_titles, audio_cloudids = check_spot(
            body)
        spot = Spot.objects.create(
            name=name, location=location, text=text, quote=quote)
        for title, cloudid in zip(image_titles, image_cloudids):
            Image.objects.create(title=title, cloudid=cloudid, spot=spot)
        ...
        spot.text = text
        spot.quote = quote
        for image in Image.objects.filter(spot=spot):
            image.delete()
